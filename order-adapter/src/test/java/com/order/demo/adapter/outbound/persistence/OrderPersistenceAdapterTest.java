...
class OrderPersistenceAdapterTest {
    ...
    void testSaveAssignsItemsList() {
        ...
        verify(repository).save(argThat(entity ->
                entity.getItems() != null && entity.getItems().size() == 2
                && entity.getItems().get(0).getSku().equals("SKU-1")
                && entity.getItems().get(1).getSku().equals("SKU-2")));
    }

    @Test
    void testFindByIdReturnsDomainObjectWithItems() {
    ...
    @Test
    void testFindByIdReturnsReservationIds() {
        OrderEntity entity = new OrderEntity("ord-1", "cust-1", "idem-1", "resv-abc",
                "CREATED", LocalDateTime.now());
        entity.setItems(List.of());
        entity.setReservationIds(List.of("resv-abc", "resv-def"));
        when(repository.findById("ord-1")).thenReturn(Optional.of(entity));
