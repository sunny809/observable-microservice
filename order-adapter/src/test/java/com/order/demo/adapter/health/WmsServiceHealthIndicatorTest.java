...
@Tag("unit")
class WmsServiceHealthIndicatorTest {
    ...
    @Test
    @DisplayName("should report UP when WMS health endpoint responds 200")
    void testHealthUpWhenReachable() {
        mockWebServer.enqueue(new MockResponse().setResponseCode(200));

        WmsServiceHealthIndicator indicator = new WmsServiceHealthIndicator(webClient);
        Health health = indicator.health();

        assertEquals("UP", health.getStatus().getCode());
        assertEquals("wms-service", health.getDetails().get("service"));
        assertEquals("reachable", health.getDetails().get("status"));
    }

    @Test
    @DisplayName("should report DOWN when WMS health endpoint is unreachable")
