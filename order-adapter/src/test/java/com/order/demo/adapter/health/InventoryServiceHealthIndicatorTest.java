...
@Tag("unit")
class InventoryServiceHealthIndicatorTest {
    ...
    @Test
    @DisplayName("should report UP when inventory health endpoint responds 200")
    void testHealthUpWhenReachable() {
        mockWebServer.enqueue(new MockResponse().setResponseCode(200));

        InventoryServiceHealthIndicator indicator = new InventoryServiceHealthIndicator(webClient);
        Health health = indicator.health();

        assertEquals("UP", health.getStatus().getCode());
        assertEquals("inventory-service", health.getDetails().get("service"));
        assertEquals("reachable", health.getDetails().get("status"));
    }

    @Test
    @DisplayName("should report DOWN when inventory health endpoint is unreachable")
